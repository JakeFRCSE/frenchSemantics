...
* **문제점 1: 신문사 크롤링 불가능**
    ...
    다양한 신문사의 기사 데이터를 자동으로 크롤링하여 대규모 데이터를 구축하려 했으나, **신문사의 정책적 크롤링 차단**으로 인해 개별 신문사 웹사이트로부터 데이터를 자동으로 수집하는 것이 불가능하다는 문제에 직면했습니다.
...
본 프로젝트의 결과를 바탕으로 다음과 같은 후속 프로젝트를 진행할 수 있습니다.
1.  **정량적 분석의 확대**: 본 프로젝트에서 시도하지 못한 통계적 분석을 본격적으로 도입하여, '국민'의 각 의미 유형별 사용 빈도, 시기별 변화 추이, 서로 다른 정치 성향의 매체별 사용 패턴 차이 등을 분석할 필요가 있습니다. 
